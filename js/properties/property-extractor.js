...
export class PropertyExtractor {
     ...
     */
    static extractElementProperties(svgElement, componentData) {
     ...
     */
    static detectComponentType(svgElement, componentData) {
        // Priority 1: Component metadata
        if (componentData?.metadata?.type) {
            return componentData.metadata.type;
        }
        
        // Priority 2: data-type attribute
        const dataType = svgElement.getAttribute('data-type');
        if (dataType) return dataType;
        
        // Priority 3: Class name inference
        const type = this.inferTypeFromClasses(svgElement) ||
                     this.inferTypeFromId(svgElement) ||
                     this.inferTypeFromContent(svgElement);
        
        if (type) return type;
        
        console.log(`[PropertyExtractor] Could not determine type for ${svgElement.id || 'unknown'}`);
        return 'unknown';
    }
     ...
     * @returns {string|null} Inferred type or null
     */
    static inferTypeFromContent(svgElement) {
